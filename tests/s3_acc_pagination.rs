...
use arx::vfs::{Location, ProviderContinuation, ProviderListingPage, ProviderRegistry, S3ObjectRef};
use std::sync::Arc;
...
async fn upload(reg: &ProviderRegistry, target: &str, key: &str, data: &[u8]) {
    ...
    let plan = TransferPlan {
        ...
        s3_spec: Some(S3TransferSpec::UploadOne {
            ...
            destination: S3ObjectRef { target: target.to_string(), bucket: "arxtest".to_string(), key: key.to_string() },
        }),
    };
    let cancel = Arc::new(AtomicBool::new(false));
    let out = execute_transfer(&plan, &[key.to_string()], reg, cancel, |_| {}).await.expect("upload");
...
async fn delete_all(reg: &ProviderRegistry, target: &str, run: &str) {
    for i in 0..COUNT {
        ...
        reg.delete_s3_at(&root(target), &key).await.expect("cleanup");
    }
}
...
async fn run_pagination(reg: &ProviderRegistry, target: &str) {
    ...
    let page1: ProviderListingPage = reg.list_page(&scoped(&run, target), None).await.expect("page1");
    ...
    let cont1 = page1.continuation.expect("continuation present after page1");
    assert!(!cont1.token.is_empty(), "continuation token non-empty");
    // one next-page request
    let page2: ProviderListingPage = reg
        .list_page(&scoped(&run, target), Some(&ProviderContinuation { token: cont1.token.clone() }))
        .await
        .expect("page2");
    ...
    assert!(page2.continuation.is_none() || page2.continuation.as_ref().unwrap().token.is_empty(),
    ...
    assert_eq!(items.len() as u32, COUNT, "no missing, exact count across pages");
...
async fn emulator_pagination() {
    let Some(reg) = s3_acceptance::maybe_skip_emulator() else { return; };
