//! MinIO physical acceptance (S3-66 target + S3-67 transfer subset).
//!
//! Gated behind ARX_MINIO_TEST=1. Runs against disposable `arx-minio-test`.
//! Exercises the SAME production S3Provider/runtime as AWS — never a
//! MinIO-special executor. Classify results PHYSICAL PASS for MinIO only;
//! this is NOT an AWS SUPPORTED claim.
...
/// Listing location scoped to a sub-prefix (no trailing slash; the S3 provider
/// adds the delimiter itself). `sub` is relative to the disposable run prefix.
...
/// List ALL entries under a scoped prefix by following continuation tokens.
